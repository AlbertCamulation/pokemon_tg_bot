...
async function setBannedUsers(data, env) {
  if (!env.POKEMON_KV) return
  await env.POKEMON_KV.put(BANNED_UID_KEY, JSON.stringify(data))
}

// --- 以下是為了相容舊代碼 (如果還有地方用到 getBannedUserIds) ---
// 如果您確定所有地方都改用 getBannedUsers 了，這兩個可以刪除
...
async function handlePokemonSearch(chatId, userId, query, env, ctx) {
  ...
  try {
    const [resTrans, resMoves, resEvents] = await Promise.all([fetchWithCache(getDataUrl('data/chinese_translation.json'), env, ctx), fetchWithCache(getDataUrl('data/move.json'), env, ctx), fetchWithCache(getDataUrl('data/events.json'), env, ctx)])
    ...
    initialMatches.forEach((p) => {
      if (p.family && p.family.id) familyIds.add(p.family.id)
    })
      ...
      list.forEach((p, rankIndex) => {
        if (ids.has(p.speciesId.toLowerCase())) {
          ...
          if (fastMoveId) {
            ...
            const charged = chargedArray
              .filter((m) => m)
              .map((m) => formatMove(m, eliteList))
              .join(', ')
    ...
    if (foundInTrash) {
      ...
      options.inline_keyboard = [[{ text: `♻️ 將 "${foundInTrash.speciesName}" 移出垃圾清單`, callback_data: `untrash_btn_${foundInTrash.speciesName}` }]]
    }
...
async function handleCurrentLeagues(chatId, env, ctx) {
  ...
  try {
    ...
    try {
      if (loadingMsg && loadingMsg.result) {
        await fetch(`https://api.telegram.org/bot${env.ENV_BOT_TOKEN}/deleteMessage`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ chat_id: chatId, message_id: loadingMsg.result.message_id }),
        })
      }
    } catch (e) {}
    ...
    await sendMessage(chatId, `❌ 處理失敗: ${e.message}`, null, env)
  }
}
async function handleLeagueCommand(chatId, command, limit = 50, env, ctx) {
  ...
  try {
    const [resRank, resTrans] = await Promise.all([fetchWithCache(getDataUrl(leagueInfo.path), env, ctx), fetchWithCache(getDataUrl('data/chinese_translation.json'), env, ctx)])
    ...
    return sendMessage(chatId, `Error: ${e.message}`, null, env)
  }
}
// --- Meta 分析 ---
...
async function handleMetaAnalysis(chatId, env, ctx) {
  const targetLeagues = [leagues.find((l) => l.command === 'great_league_top'), leagues.find((l) => l.command === 'ultra_league_top'), leagues.find((l) => l.command === 'master_league_top')]
  ...
  for (const league of targetLeagues) {
    ...
    try {
      ...
      await sendMessage(chatId, `⚠️ ${league.name} 分析錯誤: ${e.message}`, { parse_mode: '' }, env)
    }
  }
}
...
async function onCallbackQuery(callbackQuery, env, ctx) {
  ...
  const isInAdminGroup = adminGroupId && String(chatId) === adminGroupId
  ...
  if (data.startsWith('unban_btn_')) {
    ...
    const targetUid = data.replace('unban_btn_', '') // 這裡是字串 Key
    const bannedMap = await getBannedUsers(env)

    // 檢查並刪除 Key
    if (bannedMap.hasOwnProperty(targetUid)) {
      delete bannedMap[targetUid] // 刪除該用戶
      ...
      if (remainingIds.length === 0) {
        await editMessage(chatId, callbackQuery.message.message_id, '🎉 黑名單已清空！', null, env)
      } else {
        const newKeyboard = remainingIds.map((uid) => {
  ...
  if (data.startsWith('approve_uid_') || data.startsWith('ban_uid_')) {
    ...
    // ★ 嘗試從訊息文字中抓取名字 (User: Name) ★
    const msgText = callbackQuery.message.text || ''
    // Regex: 尋找 "使用者: " 後面的文字，直到換行或括號
    ...
    if (data.startsWith('approve_uid_')) {
      ...
      // 新增或更新該 UID 的名字
      bannedMap[targetUid] = targetName
      await setBannedUsers(bannedMap, env)
  ...
  switch (data) {
    ...
    case 'trash_list':
      return handleTrashCommand(chatId, userId, callbackQuery.from, env)
    case 'help_menu':
    ...
    case 'current_leagues':
      return handleCurrentLeagues(chatId, env, ctx)
    default:
      return
  }
}
...
async function onMessage(message, env, ctx) {
  ...
  if (isSuperAdmin || isInAdminGroup) {
    ...
    // 3. 檢查黑名單 (資料結構變為 Object，改用 Key 檢查)
    const bannedMap = await getBannedUsers(env)
    if (bannedMap[userId]) return // 存在 Key 即為黑名單
    ...
    if (!allowedIds.includes(userId)) {
      ...
      if (adminGroupId) {
        ...
        const safeUser = escapeHtml(username ? `@${message.from.username}` : '無') // 修正 username 顯示

        // 注意：這裡我們在通知文字中保留了名字，稍後按按鈕時會用到
        ...
        await sendMessage(env.ADMIN_GROUP_UID, adminMsg, adminOptions, env)
      }
      return
    }
  }
  ...
  if (command) {
    switch (command) {
      ...
      case 'banlist':
        ...
        const bannedMap = await getBannedUsers(env)
        const bannedIds = Object.keys(bannedMap) // 取得所有 UID Keys

        if (bannedIds.length === 0) {
          return sendMessage(chatId, '🎉 目前沒有人在黑名單中。', null, env)
        }

        // 製作按鈕：顯示儲存的名字
        const keyboard = bannedIds.map((uid) => {
          const name = bannedMap[uid] || 'Unknown' // 讀取名字
          return [{ text: `🔓 解封: ${name}`, callback_data: `unban_btn_${uid}` }]
        })
...
async function getPokemonDataOnly(query, env, ctx) {
  ...
  // 2. 取得基礎資料 (改用快取函數)
  const [data, movesData, eventsData] = await Promise.all([getJsonData('trans', 'data/chinese_translation.json', env, ctx), getJsonData('moves', 'data/move.json', env, ctx), getJsonData('events', 'data/events.json', env, ctx)])
  ...
  // 4. 進化鏈處理
  const familyMembers = target.family && target.family.id ? data.filter((p) => p.family && p.family.id === target.family.id) : [target]
  ...
  const rankResults = await Promise.all(leagues.map((l) => getLeagueRanking(l, env, ctx)))
      ...
      if (ids.has(sid)) {
        ...
        if (fastMoveId) {
          ...
          const charged = cMoves
            .filter((m) => m)
            .map((m) => formatMove(m, eliteList, sid))
            .join(', ')
