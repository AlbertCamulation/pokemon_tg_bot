...
const leagues = [
  ...
  { command: "ultra_league_top", name: "究極紀念盃 (2500)", cp: "2500", path: "data/rankings_premierultra_2500.json" },
...
function getDataUrl(filename) {
  return `https://raw.githubusercontent.com/${GITHUB_USERNAME}/${REPO_NAME}/${BRANCH_NAME}/${filename}?ver=1005`;
}
...
async function getBannedUsers(env) {
  ...
  if (Array.isArray(data)) {
      const newMap = {};
      data.forEach(id => { newMap[id] = "Unknown"; });
      return newMap;
  }
  return data || {};
}
...
async function onCallbackQuery(callbackQuery, env, ctx) {
  ...
  if (data.startsWith("approve_uid_") || data.startsWith("ban_uid_")) {
      ...
      if (data.startsWith("approve_uid_")) {
          ...
          try { await sendMessage(targetUid, "✅ 管理員已開通您的權限！", null, env); } catch(e){}
  ...
  if (command) {
    switch (command) {
      ...
      case "banlist":
        ...
        return sendMessage(chatId, `💀 <b>黑名單管理</b>\n點擊按鈕解封：`, { 
            parse_mode: "HTML",
