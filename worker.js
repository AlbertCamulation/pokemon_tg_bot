...
<head>
    ...
    <style>
        ...
        #suggestionList {
            ...
            background: rgba(10, 10, 10, 0.98); border: 1px solid #660000;
            border-top: none; border-radius: 0 0 2rem 2rem;
            max-height: 350px; overflow-y: auto; box-shadow: 0 20px 50px rgba(0,0,0,0.9);
            display: none;
        }
        .suggestion-item { padding: 15px 25px; cursor: pointer; border-bottom: 1px solid #1a1a1a; transition: 0.3s; }
        .suggestion-item:hover { background: #440000; color: #ff4444; padding-left: 40px; }
        .suggestion-item:last-child { border-bottom: none; border-radius: 0 0 2rem 2rem; }
    </style>
</head>
<body class="pb-20">
    <div class="max-w-6xl mx-auto p-4 md:p-8">
        <div class="flex flex-col md:flex-row justify-between items-center mb-12 gap-6">
            <div class="flex items-center gap-4">
                <div class="w-14 h-14 btn-red rounded-2xl flex items-center justify-center text-3xl"><i class="fa-solid fa-microchip"></i></div>
                <div>
        ...
        <div id="results" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
            <div class="col-span-full text-center py-40 text-zinc-800">
                <i class="fa-solid fa-ghost fa-beat text-8xl mb-8 opacity-20"></i>
                <p class="tech-font uppercase tracking-widest text-sm">System Ready / Awaiting Target</p>
            </div>
        </div>
    </div>
   ...
   <script>
        ...
        function calculateEffectiveness(types) {
            ...
            const valid = types.filter(t => t && t.toLowerCase() !== 'none');
            valid.forEach(type => {
                const lower = type.toLowerCase();
                Object.keys(typeChart).forEach(attacker => {
                    if (typeChart[attacker][lower]) results[attacker] *= typeChart[attacker][lower];
                });
            });
            return results;
        }
            ...
            try {
                    ...
                    resultsDiv.innerHTML = \`
                        <div class="col-span-full text-center py-20 px-10">
                            ...
                            <div class="inline-block bg-red-950/30 border border-red-900 p-6 rounded-3xl text-zinc-400 max-w-lg">
                                系統查無此物種在各大聯盟的排名數據。建議直接轉送或作為收藏，不具備 PvP 投資價值。
                            </div>
                        </div>
                    \`;
                    return;
                ...
                evoDiv.innerHTML = data.evolutionChain.map((p, idx) => \`
                    <div class="flex items-center">
                        ...
                        \${idx < data.evolutionChain.length - 1 ? '<i class="fa-solid fa-angles-right mx-6 text-red-600 opacity-20 text-xl"></i>' : ''}
                    </div>
                \`).join('');
                ...
                if (data.events && data.events.length > 0) {
                    eventBanner.innerHTML = data.events.map(e => \`
                        <div class="flex items-center gap-4 text-red-200"><i class="fa-solid fa-radiation text-red-600 text-2xl"></i>
                        <div><div class="font-black text-xl uppercase tracking-tighter">EVENT_DETECTION: \${e.eventName}</div>
                        <div class="text-xs opacity-70">\${e.date} · <a href="\${e.link}" target="_blank" class="underline decoration-red-600">LINK_STUB</a></div></div></div>
                    \`).join('');
                ...
                const renderCard = (league) => \`
                    <div class="card-dark rounded-[3rem] overflow-hidden hover:scale-[1.02] transition-all duration-500 shadow-2xl border border-zinc-900">
                        <div class="p-10 bg-zinc-950/80 border-b border-zinc-900">
                            ...
                            <div class="flex items-center gap-3"><span class="w-12 h-1 bg-red-600"></span><span class="text-[9px] text-zinc-500 tech-font tracking-widest uppercase">Combat Stream Analysis</span></div>
                        </div>
                            ...
                            \${league.pokemons.map(p => \`
                                <div class="p-6 rounded-[2rem] bg-zinc-900/40 border border-zinc-800/40 group hover:bg-zinc-900/70 transition-colors shadow-lg">
                                    <div class="flex justify-between items-start mb-4">
                                        <div><span class="text-[10px] font-black text-red-500 tech-font opacity-60 tracking-widest mb-1 block">RANKING_ID #\${p.rank}</span>
                                    ...
                                    <div class="mt-4 flex justify-between items-center opacity-40 text-[9px] tech-font font-bold"><span>SECURE_LINK</span><span>SCORE_VAL: \${p.score}</span></div>
                                </div>
                            \`).join('')}
                        </div>
                    </div>
                ...
                resultsDiv.innerHTML = filtered.map(renderCard).join('') + 
                                     (others.length > 0 ? '<div class="col-span-full py-20 opacity-10 text-center tech-font text-sm tracking-[1.5em] text-zinc-800">--- END_OF_SECONDARY_BUFFERS ---</div>' : '') +
